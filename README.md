...
To measure API payload sizes before/after egress reductions, set `SUPABASE_EGRESS_LOG=1` in your server environment (`.env.local` or production). Heavy list/read routes log `[egress] <route>: <bytes>` to the server console when enabled. See `.cursor/rules/EGRESS-GUARDRAILS.mdc` for list vs detail select patterns and caching conventions.

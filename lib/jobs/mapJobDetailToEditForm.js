...
import { getJobContactDisplayFields } from './jobDetailHelpers';
...
function mapContactForEdit(contact) {
  if (!contact) return {};
  const display = getJobContactDisplayFields(contact);
  return {
    contactID: contact.contactID || contact.id || contact.contactId || '',
    contactFullname: display.name,
    firstName: contact.firstName || contact.first_name || '',
    middleName: contact.middleName || contact.middle_name || '',
    lastName: contact.lastName || contact.last_name || '',
    phoneNumber: display.officePhone,
    mobilePhone: display.mobilePhone,
    email: display.email,
  };
}
...
export function mapJobDetailToEditForm(payload) {
  ...
  const jobStatus = rawStatus != null ? String(rawStatus).trim() : '';

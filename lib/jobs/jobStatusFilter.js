...
export function getJobStatusFilterDbValues(filterStatus, jobStatusesList) {
  ...
  const isUnconfirmed =
    canonical === '554' ||
    canonicalNorm === 'UNCONFIRMED' ||
    canonicalNorm === 'CREATED';
  if (isUnconfirmed) {
    for (const alias of ['554', 'Unconfirmed', 'UNCONFIRMED', 'CREATED', 'Created', 'PENDING']) {
      values.add(alias);
    }
  }
  ...
  const isInProgress =
    ...
    canonicalNorm === 'IN_PROGRESS' ||
    canonical === '-5';
  if (isInProgress) {
    for (const alias of ['IN_PROGRESS', 'In Progress', '-5']) {
      values.add(alias);
    }
  }
  ...
  const isJobDone =
    canonical === '-1' ||
    canonicalNorm === 'JOB_DONE' ||
    canonicalNorm === 'COMPLETED';
  if (isJobDone) {
    for (const alias of ['-1', 'Job Done', 'COMPLETED', 'Completed', 'JOB_COMPLETE']) {
      values.add(alias);
    }
  }

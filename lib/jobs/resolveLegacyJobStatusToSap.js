...
export function resolveLegacyStatusToSapId(rawStatus, sapIndex) {
  ...
  // Portal IN_PROGRESS / "In Progress" → SAP ID -5 (Worker on the Way).
  // UDT has no exact "In Progress" label; do not keyword-match *PROGRESS*.
  const compact = normalized.replace(/\s+/g, '');
  if (PORTAL_IN_PROGRESS_ALIASES.has(normalized) || PORTAL_IN_PROGRESS_ALIASES.has(compact)) {
    if (sapIndex.byId.has('-5')) {
      return {
        kind: 'matched',
        raw,
        normalized,
        id: '-5',
        label: sapIndex.byId.get('-5') || null,
        reason: 'portal_alias_in_progress_id',
      };
    }
    return { kind: 'unknown', raw, normalized, candidates: [] };
  }

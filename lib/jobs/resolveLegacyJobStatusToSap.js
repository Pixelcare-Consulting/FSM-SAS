...
function tryExplicitPortalAliases(normalized, sapIndex) {
  ...
  if (PORTAL_IN_PROGRESS_ALIASES.has(normalized) || PORTAL_IN_PROGRESS_ALIASES.has(compact)) {
    const hit = matchSingleId(findIdsByLabelKeyword(sapIndex, 'PROGRESS'), sapIndex, 'portal_alias_in_progress');
    if (hit) return hit;
  }
...
export function resolveLegacyStatusToSapId(rawStatus, sapIndex) {
  ...
  if (tokens.length) {
    const candidateIds = [];
    for (const [id, labelNorm] of sapIndex.byIdNorm.entries()) {
      if (tokensContained(tokens, labelNorm)) candidateIds.push(id);
    }
    if (candidateIds.length === 1) {
      ...
      return {
        ...
        id,
        label: sapIndex.byId.get(id) || null,
        reason: 'token_contains_all',
      };
    }
    if (candidateIds.length > 1) {
      return { kind: 'ambiguous', raw, normalized, candidates: candidateIds, reason: 'token_contains_all_multi' };
    }
  }

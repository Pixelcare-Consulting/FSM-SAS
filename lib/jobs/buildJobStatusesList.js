...
/** Portal/mobile workflow states — not SAP UDT rows. Prepended when SAP list is present. */
const PORTAL_ONLY_EXTRAS = [
...
function prependPortalOnlyExtras(sapRows) {
  const existing = new Set(
    sapRows.map((row) => String(row.value ?? '').trim().toUpperCase()).filter(Boolean)
  );
  const extras = PORTAL_ONLY_EXTRAS.filter((extra) => !existing.has(extra.value));
  return [...extras, ...sapRows];
}

/**
 * Build the job-status dropdown list.
 * When SAP rows exist: prepend portal-only Created and In Progress, then SAP U_JobStatusID
 * as value with SAP U_JobStatus as name; Settings color overlay only. Settings cannot rename SAP IDs.
 *
...
export function buildJobStatusesList({ settingsTypes = null, sapRows = [], sapSnapshot = [] } = {}) {
  ...
  if (normalizedSap.length > 0) {
    return applySettingsColorOnly(prependPortalOnlyExtras(normalizedSap), settingsTypes);
  }

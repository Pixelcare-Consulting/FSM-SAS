...
import { formatSingaporeTime, getSingaporeUtcDayRange } from '../utils/singaporeDateTime';
...
/** Normalize YYYY-MM-DD query values; invalid or empty → null. */
export function normalizeJobHistoryDateParam(value) {
 ...
 * Optional dateFrom/dateTo (YYYY-MM-DD) filter on scheduled_start in Asia/Singapore.
 */
export async function fetchCustomerJobHistoryPage(supabase, customerUUID, options = {}) {
  ...
  let dateFrom = normalizeJobHistoryDateParam(options.dateFrom);
  let dateTo = normalizeJobHistoryDateParam(options.dateTo);
  ...
  const { data, totalCount } = await paginatedSelect(
    ...
    CUSTOMER_JOB_HISTORY_LIST_SELECT,
    {
      ...
      filters: (query) => {
        ...
        if (dateFrom) {
          const { start } = getSingaporeUtcDayRange(dateFrom);
          q = q.gte('scheduled_start', start.toISOString());
        }
        if (dateTo) {
          const { end } = getSingaporeUtcDayRange(dateTo);
          q = q.lte('scheduled_start', end.toISOString());
        }
        return q;
      },
    }

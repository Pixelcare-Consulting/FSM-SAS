...
/**
 * Delete read receipts for the given message ids (marks them unread for the user).
 * @param {import('@supabase/supabase-js').SupabaseClient} supabase
 * @param {string} userId
 * @param {string[]} messageIds
 */
export async function markJobMessagesUnread(supabase, userId, messageIds) {
  const unique = [...new Set((messageIds || []).filter(Boolean).map(String))];
  if (!supabase || !userId || unique.length === 0) {
    return { unmarked: 0 };
  }
  ...
  for (let i = 0; i < unique.length; i += chunkSize) {
    ...
    unmarked += typeof count === 'number' ? count : batch.length;
  }
...
/**
 * Mark all (non-deleted) messages for a job as unread for the user (delete receipts).
 * @param {import('@supabase/supabase-js').SupabaseClient} supabase
 * @param {string} userId
 * @param {string} jobId
 */
export async function markJobThreadUnread(supabase, userId, jobId) {
  if (!supabase || !userId || !jobId) return { unmarked: 0 };
  ...
  return markJobMessagesUnread(supabase, userId, ids);
}

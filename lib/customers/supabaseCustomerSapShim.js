...
export function sapPartnerFromSupabaseCustomerBundle(c) {
  ...
  const bill =
    BPAddresses.find(
      (a) => billDefaultName && a.AddressName === billDefaultName && isBillAddressType(a.AddressType)
    ) ||
    BPAddresses.find((a) => isBillAddressType(a.AddressType));
  const ship =
    BPAddresses.find(
      (a) => shipDefaultName && a.AddressName === shipDefaultName && isShipAddressType(a.AddressType)
    ) ||
    BPAddresses.find((a) => isShipAddressType(a.AddressType));
  ...
  return {
    ...
    ...(primaryEmployeeContact
      ...
      : {}),
    BilltoDefault: billDefaultName || bill?.AddressName || first?.AddressName || '',
    ShiptoDefault: shipDefaultName || ship?.AddressName || '',

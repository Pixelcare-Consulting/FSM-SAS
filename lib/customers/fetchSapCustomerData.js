...
import { filterActiveLocationOptions } from '../jobs/filterActiveLocationOptions.js';
import { withLocationOptionsStatuses } from '../jobs/locationOptionAddressStatus.js';
import { fetchAddressDetailsMaps } from './addressDetailsMaps.js';
import {
...
import { withSapSessionRetry } from '../services/sapSessionRetry';
import {
...
function normalizeKeepSiteIdList({ keepSiteId, keepSiteIds } = {}) {
  const raw = [];
  if (Array.isArray(keepSiteIds)) raw.push(...keepSiteIds);
  else if (keepSiteIds != null && keepSiteIds !== '') raw.push(keepSiteIds);
  if (keepSiteId != null && keepSiteId !== '') raw.push(keepSiteId);
  return [...new Set(raw.map((id) => String(id ?? '').trim()).filter(Boolean))];
}

/**
 * Attach customer_address_details.status onto job-form location options.
 * Missing detail rows stay Active.
 */
async function attachAddressDetailsStatusToLocations(cardCode, locations) {
  if (!Array.isArray(locations) || locations.length === 0) return [];

  try {
    const supabase = getSupabaseAdmin();
    const maps = await fetchAddressDetailsMaps(supabase, cardCode);
    return withLocationOptionsStatuses(locations, maps);
  } catch (err) {
    console.warn(
      'fetchLocationsByCardCode address-details status:',
      err?.message || err
    );
 ...
 *   keepSiteId?: string,
 *   keepSiteIds?: string|string[],
 *   filterInactive?: boolean,
 * }} [options]
 */
export async function fetchLocationsByCardCode(
  cardCode,
  { req, keepSiteId, keepSiteIds, filterInactive = true } = {}
) {
  const code = String(cardCode || '').trim();
  ...
  if (locations.length === 0) {
    try {
      ...
      if (fromLeadMasterlist.length > 0) {
        source = 'sap_lead_masterlist';
        locations = fromLeadMasterlist;
      }
  ...
  const withStatus = await attachAddressDetailsStatusToLocations(code, locations);
  if (!filterInactive) {
    return { source, locations: withStatus };
  }

  return {
    source,
    locations: filterActiveLocationOptions(withStatus, {
      keepSiteIds: normalizeKeepSiteIdList({ keepSiteId, keepSiteIds }),
    }),
  };
}
...
export async function fetchServiceCallsByCardCode(
  ...
  try {
    return await withSapSessionRetry(sessionCookies, async (cookies) => {
      ...
      for (const card of uniqueCodes) {
        const result = await fetchServiceCallsForSingleCardCode(card, cookies);

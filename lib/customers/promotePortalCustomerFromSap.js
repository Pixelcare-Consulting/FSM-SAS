...
function isPortalCustomerCode(code) {
  return /^CP\d+$/i.test(normalizeCode(code));
}

/** Official SAP customer CardCodes only (not L leads). */
function isOfficialSapCustomerCode(code) {
  return /^C[A-Z0-9]+$/.test(normalizeCode(code));
}
...
function isSyncedPortalCpRow(row) {
...
export async function promotePortalCustomerFromSap({
  ...
  const portalCode = normalizeCode(portalCustomerCode);
  const sapCode = normalizeCode(sapCardCode);

  if (!isPortalCustomerCode(portalCode)) {
    throw new Error(`Invalid portal customer code: ${portalCustomerCode} (expected CP#####)`);
  }

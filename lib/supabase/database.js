...
import { NOTIFICATIONS_SUMMARY_SELECT } from '../notifications/notificationSummary';

/**
 * Server admin client: reuse `getSupabaseAdmin` from `./server` (process singleton).
 * Lazy `require` so browser bundles that import this module for client-side helpers
 * do not pull SERVICE_ROLE into the client graph.
 *
 * Fallback: if Turbopack stubs `./server` to an empty module, create a local
 * singleton with the same credentials/options so API routes still work.
 */
...
let _fallbackAdminSingleton = null;
...
function getSupabaseAdminClient() {
  ...
  try {
    ...
    if (typeof serverMod.getSupabaseAdmin === 'function') {
      return serverMod.getSupabaseAdmin();
    }
  } catch {
  ...
  if (!_fallbackAdminSingleton) {
    const createClient = getCreateClientFn();
    _fallbackAdminSingleton = createClient(supabaseUrl, supabaseServiceRoleKey, {
      auth: {
  ...
  return _fallbackAdminSingleton;
}

function createServerClientBound(accessToken) {
  try {
    const serverMod = require('./server');
    if (typeof serverMod.createServerClient === 'function') {
      return serverMod.createServerClient(accessToken);
    }
  } catch {
    // fall through
  }
...
/** job_tasks rows for a single job (assembled into job detail payloads). */
...
/** Slim technician_jobs embed for job detail — avoid technician(*, user(*)). */
export const JOB_TECHNICIAN_JOBS_SELECT = `
  id,
  job_id,
  technician_id,
  assignment_status,
...
/** Slim job_equipments + equipment for detail/PDF. */
export const JOB_EQUIPMENTS_SELECT = `
  id,
  job_id,
  equipment_id,
  quantity_used,
...
/**
 * Job detail is assembled via header + parallel slim child queries (assembleJobDetail).
 * Nested PostgREST embeds for technician(*, user(*)) are intentionally avoided.
 */
...
/**
 * Header + parallel slim child queries (tasks, technician_jobs, equipments).
 * Same response shape as the old nested JOB_DETAIL_SELECT without one deep PostgREST embed.
 */
async function assembleJobDetail(db, headerRow) {
  if (!headerRow?.id) return null;
  const jobId = headerRow.id;

  const [technicianJobsResult, tasksResult, equipmentsResult] = await Promise.all([
    db
      .from('technician_jobs')
      .select(JOB_TECHNICIAN_JOBS_SELECT)
      .eq('job_id', jobId)
      .is('deleted_at', null),
    db
      .from('job_tasks')
    ...
    db.from('job_equipments').select(JOB_EQUIPMENTS_SELECT).eq('job_id', jobId),
  ]);

  if (technicianJobsResult.error) throw technicianJobsResult.error;
  if (tasksResult.error) throw tasksResult.error;
  if (equipmentsResult.error) throw equipmentsResult.error;

  return filterActiveTechnicianJobs({
    ...headerRow,
    technician_jobs: technicianJobsResult.data || [],
    job_tasks: tasksResult.data || [],
    job_equipments: equipmentsResult.data || [],
  });
}
...
export const jobService = {
  ...
  async findById(id, client = null) {
    ...
    return assembleJobDetail(db, data);
  },
  ...
  /** Active technician_jobs for one job (slim columns + technician). */
  async findTechnicianJobsByJobId(jobId, client = null) {
    const db = client || getClient();
    const { data, error } = await db
      .from('technician_jobs')
      .select(JOB_TECHNICIAN_JOBS_SELECT)
  ...
  /** job_equipments + equipment for one job. */
  async findEquipmentsByJobId(jobId, client = null) {
    const db = client || getClient();
    const { data, error } = await db
      .from('job_equipments')
      .select(JOB_EQUIPMENTS_SELECT)
      .eq('job_id', jobId);
...
export const customerService = {
   ...
   * Mirrors getNextJobNumber: ordered max (bounded CP00000–CP99999) then existence check
   * including soft-deleted rows (UNIQUE is not limited to active customers).
  ...
  async getNextPortalCardCode(client = null) {
    ...
    // Lexicographic max matches numeric order for fixed 5-digit CP##### codes.
    // Include soft-deleted so we do not re-propose a code still held by UNIQUE.
    ...
    for (let attempt = 0; attempt < 20; attempt += 1) {
      const candidate = 'CP' + String(nextNum).padStart(5, '0');
...
export const notificationService = {
  ...
  async getByWorkerIds(workerIds, limit = 10, client = null) {
    ...
    const { data, error } = await db
      .from('notifications')
      .select(NOTIFICATIONS_SUMMARY_SELECT)
      .or(orClause)
      .eq('hidden', false)
      .order('created_at', { ascending: false })
      .limit(limit);

...
import { allocateNextPortalCustomerCodes } from './portalCustomerCodes';
...
export const customerService = {
   ...
   * Numeric max of /^CP\d+$/ (not string-max in a 5-digit window), then skip occupied
  ...
  async getNextPortalCardCode(client = null) {
    const db = client || getClient();
    const codes = await allocateNextPortalCustomerCodes(db, 1);
    return codes[0];
  },

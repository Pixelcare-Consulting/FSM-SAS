...
export const userService = {
  ...
  async findById(id, client = null) {
    const db = client || getClient();
    // maybeSingle avoids PostgREST 406 when zero rows match (e.g. soft-deleted user).
    const { data, error } = await db
      .from('users')
      .select(`
        *,
...
export const customerService = {
  ...
  async getGenericCustomers(client = null) {
    ...
    const { data, error } = await db
      .from('customer')
      .select('id, customer_code, customer_name, customer_address, phone_number, email, synced_to_sap_at, block, unit, notes, created_at, updated_at')
      .eq('source', 'portal')
      .is('deleted_at', null)
  ...
  async getSapMasterlistCustomers(client = null) {
    ...
    for (let p = 0; p < MAX_PAGES; p += 1) {
      ...
      const { data, error } = await db
        .from('customer')
        .select(
          'id, customer_code, customer_name, customer_address, phone_number, email, source, synced_to_sap_at, created_at, updated_at'
        )

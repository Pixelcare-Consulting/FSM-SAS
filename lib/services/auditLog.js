...
function buildIncentiveScheduleDetails(incentiveResult) {
  ...
  if (schedule.U_JobTech != null) details.U_JobTech = schedule.U_JobTech;
  if (schedule.getError) details.getError = schedule.getError;
  return details;
}
...
export function writeJobEmailAudit({
  ...
  // Job assignment emails (especially skipped/WARNING) spam the audit log.
  // Re-enable by uncommenting the write below.
  if (templateKey === 'jobAssigned') {
    // if (req) {
...
/**
 * Normalize archive cutoff to an exclusive ISO timestamp (created_at < before).
 * Accepts YYYY-MM-DD or full ISO. Date-only → start of that UTC day.
 */
export function normalizeArchiveBefore(before) {
...
/**
 * Count audit_logs rows older than / at-or-after a cutoff (preview for archive).
 */
export async function previewAuditLogsArchive({
  supabase: supabaseClient,
  before,
} = {}) {
  const beforeIso = normalizeArchiveBefore(before);
  if (!beforeIso) {
    return { ok: false, error: 'A valid cutoff date (before) is required' };
  }
  if (Date.parse(beforeIso) >= Date.now()) {
    return { ok: false, error: 'Cutoff must be in the past' };
  }

  const supabase = supabaseClient || getSupabaseAdmin();

  const [{ count: toDelete, error: delErr }, { count: toKeep, error: keepErr }] =
    await Promise.all([
      supabase
        .from('audit_logs')
...
async function deleteAuditLogsBatch(supabase, beforeIso, batchSize) {
...
/**
 * Hard-delete audit_logs with created_at < before (batched).
 * Writes a follow-up AUDIT_LOGS_ARCHIVE row after deletes complete.
 */
export async function archiveAuditLogs({
  ...
  const preview = await previewAuditLogsArchive({ supabase: supabaseClient, before });
  ...
  if (preview.toDelete === 0) {
    return {
      ...
      deleted: 0,
      toKeep: preview.toKeep,
    };
  }

  const supabase = supabaseClient || getSupabaseAdmin();
  const size = Math.min(Math.max(Number(batchSize) || ARCHIVE_BATCH_SIZE, 100), 5000);
  let deleted = 0;

  while (true) {
    const n = await deleteAuditLogsBatch(supabase, preview.before, size);
    if (n === 0) break;
    deleted += n;
    if (n < size) break;
  }
  ...
  const auditFields = {
    action: AUDIT_ACTIONS.AUDIT_LOGS_ARCHIVE,
    category: AUDIT_CATEGORIES.SYSTEM,
    entityType: 'audit_logs',
    entityId: preview.before,
    entityLabel: `Archive before ${preview.before}`,
    description: `Archived ${deleted.toLocaleString()} audit log(s) older than ${preview.before}`,
    details: {
      before: preview.before,
      deleted,
      previewToDelete: preview.toDelete,
      toKeep: preview.toKeep,
    },
    status: AUDIT_STATUS.SUCCESS,
  ...
  if (req) {
    await writeAuditLogFromRequest(req, auditFields);
  } else {
    await writeAuditLog({
      supabase,
      userId,

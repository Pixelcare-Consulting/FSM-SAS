...
export function buildJobSyncSapPlan({ job, customer, scheduleRows, jobCategory }) {
  ...
  if (!job.job_number) {
    ...
    jobScheduleStep.endpoint = `${getApiJobScheduleEntityName()}?$filter=Name or U_JobNo`;
    ...
    jobScheduleStep.note =
      'Lookup by Name/U_JobNo then PATCH SAP numeric Code; POST only if no row. Preview Name uses placeholder activity.';
  } else {
    const built = buildApiJobSchedulePayload({
      jobNumber: job.job_number,
    ...
    jobScheduleStep.patchBody = built.patchBody;
    jobScheduleStep.note =
      'Lookup by Name/U_JobNo then PATCH all matching numeric Codes; POST only if no row exists (no string Code).';
    jobScheduleStep.mappings = buildScheduleMappings(
      job,
      serviceCallNumber,
      sapTechCode,
  ...
  if (!serviceCallNumber) {
    ...
    serviceCallStep.note = 'LineNum may change after GET existing ServiceCallActivities in live sync. Live PATCH sends only this job activity line.';
    ...
    serviceCallStep.payload = buildServiceCallPatchBody(line);
    serviceCallStep.mappings = buildServiceCallMappings(job, poNumber, line);
    serviceCallStep.note = 'Live sync GETs ServiceCalls first, then PATCH only this job activity line (LineNum + ActivityCode + UDFs).';
  }

  const recontactDate = pickJobIncentiveRecontactDate(job, scheduleRows);
  const scl5Preview =
    job.sap_activity_id &&
    `UPDATE [SCL5] SET U_API_JobNumber, U_CMNumber, U_JobIncome, U_JobStatus, U_CMStatus WHERE ClgID = ${job.sap_activity_id}`;
...
function buildServiceCallMappings(job, poNumber, line) {
  return [
    ...
    mapRow('(portal always NI)', 'U_JobStatus', 'NI', line.U_JobStatus),
  ];
}

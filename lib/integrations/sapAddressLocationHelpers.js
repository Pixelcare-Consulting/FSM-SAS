...
/** Match portal ship rows whose site_id uses a ` - 1` suffix to SAP AddressName. */
export function findExistingLocationRow(existingRows, locationRow) {
  ...
  const siteId = locationRow.site_id;
  ...
  if (addrType !== 'bo_ShipTo') return null;
  ...
  if (siteId.endsWith(SHIP_MINUS_ONE_SUFFIX)) {
    ...
    existing =
      existingRows.find(
        (row) =>
          row.site_id === base && normalizePortalAddressType(row.address_type) === 'bo_ShipTo'
      ) || null;
  }
  return existing;
}

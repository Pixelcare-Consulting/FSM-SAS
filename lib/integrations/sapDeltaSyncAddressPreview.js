 ...
 * @returns {{ label: string, before: string|null, after: string|null, action: 'add'|'update'|'unchanged'|'remove' }[]}
 */
...
export async function enrichPreviewItemsWithAddresses({ supabase, sessionCookies, items }) {
  ...
  for (const item of addressable) {
    ...
    const addressChanges = computeAddressChangesForEntity(existingRows, bpAddresses);

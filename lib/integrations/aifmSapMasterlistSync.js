...
function normPortalAddressContent(row) {
...
async function dedupeStalePortalLocations({
  ...
  const skipStaleDeletes =
    ...
    (addressFetchMeta?.sqlCount ?? sapBillShipCount) < portalBillShipCount;

  if (skipStaleDeletes) {
    console.warn(
      `[dedupeStalePortalLocations] Skipping stale portal deletes — SAP bill/ship count (${sapBillShipCount}) ` +
        `lower than portal (${portalBillShipCount}). meta=${JSON.stringify(addressFetchMeta || {})}`
    );
  }
  ...
  const shipRows = existingRows.filter(
    (row) => normalizePortalAddressType(row.address_type) === 'bo_ShipTo',
  );
  const shipByContent = new Map();
  for (const row of shipRows) {
    const contentKey = normPortalAddressContent(row);
    if (!contentKey) continue;
    if (!shipByContent.has(contentKey)) shipByContent.set(contentKey, []);
    shipByContent.get(contentKey).push(row);
  }

  for (const group of shipByContent.values()) {
    if (group.length < 2) continue;
    const preferred =
      group.find((row) => sapSiteIds.has(row.site_id)) ||
      group.find((row) => row.location_id) ||
      group[0];
    for (const row of group) {
      ...
      if (delErr) throw new Error(`${tableName} delete duplicate ship content: ${delErr.message}`);
      removed += 1;
...
export function buildCustomerFieldsFromDetails(customer_name, details, { isInsert = false, existingAddress = '' } = {}) {
  ...
  if (details.billToDefault) row.bill_to_default = details.billToDefault;
  if (details.shipToDefault) row.ship_to_default = details.shipToDefault;
...
export async function upsertCustomerLocationsFromSap(
  ...
  const { data: allExisting, error: listErr } = await supabase
    ...
    .select('id, site_id, street, building, block, city, country_name, zip_code, address, address_type')

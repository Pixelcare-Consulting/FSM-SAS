...
/** Slim select for scheduler grid cards — site contact lazy-loaded via job-site-contact API. */
...
export const CUSTOMER_LOCATION_SCHEDULER_SELECT =
...
export function buildSchedulerEventsPayload({
  ...
  return (async () => {
    ...
    if (customerIds.length > 0) {
      try {
        customerLocationsByCustomerId = await fetchCustomerLocationsByCustomerIds(
          supabase,
          customerIds
        );
      } catch (customerLocErr) {
        console.warn(
          "[SchedulerAPI] Failed to fetch customer_location for jobs:",

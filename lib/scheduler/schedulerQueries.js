...
/**
 * Hard cap per dated jobs query (in-range + overlap). Matches PostgREST default max-rows
 * so an unbounded window cannot silently pull more than one page of rows.
 */
export const SCHEDULER_DATED_JOBS_LIMIT = 1000;
/** Reject absurd client ranges (month view + pad is ~45–62 days). */
export const SCHEDULER_MAX_RANGE_DAYS = 62;
...
/**
 * Clamp or reject a scheduler date window. Returns null when range is invalid.
 * @returns {{ rangeStart: string, rangeEnd: string, clamped: boolean } | null}
 */
export function normalizeSchedulerRange(rangeStart, rangeEnd, maxDays = SCHEDULER_MAX_RANGE_DAYS) {
  ...
  if (endMs - startMs <= maxMs) {
    return { rangeStart, rangeEnd, clamped: false };
  }
  return {
    rangeStart,
    rangeEnd: new Date(startMs + maxMs).toISOString(),
    clamped: true,
  };
}
...
export async function fetchJobsForSchedulerWindow(
  ...
  rangeEnd,
  selectFragment,
  { includeUndated = false, datedLimit = SCHEDULER_DATED_JOBS_LIMIT } = {}
) {
  const byId = new Map();
  const addRows = (rows) => {
  ...
  const limit = Math.max(1, Number(datedLimit) || SCHEDULER_DATED_JOBS_LIMIT);

  // Query A: jobs whose start falls in the window.
  // Query B: overlap — started before window but still open at rangeStart (needs scheduled_end index).
  const queries = [
    supabase
      .from("jobs")

...
/** With Realtime connected, poll is a soft safety net — longer interval reduces full window reloads. */
const POLL_INTERVAL_REALTIME_MS = 5 * 60 * 1000;
  ...
  useEffect(() => {
    ...
    const restartPoll = () => {
      ...
      pollRef.current = setInterval(() => {
        if (document.visibilityState !== "visible") return;
        // Prefer current-window soft revalidate; avoid stampeding every cached range.

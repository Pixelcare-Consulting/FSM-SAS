...
function schedulerEventIdentity(event) {
  const id = event?.technicianJobId ?? event?.id ?? event?.event_id;
  return id == null ? null : String(id);
}

/**
 * Update one event inside a cached window payload in place so a later
 * stale-while-revalidate fetch cannot wipe a newer local reassign.
 */
export function patchSchedulerWindowCacheEvent(key, updatedEvent) {
  if (!key || !updatedEvent) return false;
  const cached = readSchedulerCache(key, WINDOW_DATA_TTL_MS);
  if (!cached || !Array.isArray(cached.events)) return false;

  const id = schedulerEventIdentity(updatedEvent);
  if (id == null) return false;

  const resourceId = updatedEvent.resourceId ?? updatedEvent.technicianId;
  const technicianId = updatedEvent.technicianId ?? updatedEvent.resourceId;
  let found = false;
  const events = cached.events.map((evt) => {
    if (schedulerEventIdentity(evt) !== id) return evt;
    found = true;
    return {
      ...evt,
      ...updatedEvent,
      resourceId: resourceId ?? evt.resourceId,
      technicianId: technicianId ?? evt.technicianId,
      meta: {
        ...(evt.meta || {}),
        ...(updatedEvent.meta || {}),
      },
  ...
  writeSchedulerCache(key, {
    ...cached,
    events: found ? events : [...cached.events, updatedEvent],
  });
  return true;
}

/**
 * Replace every cached window event for a job so a later stale-while-revalidate
 * fetch cannot restore technicians that were just removed.
 */
export function replaceSchedulerWindowCacheEventsForJob(key, jobId, nextEvents) {
  if (!key || jobId == null) return false;
  const cached = readSchedulerCache(key, WINDOW_DATA_TTL_MS);
  if (!cached || !Array.isArray(cached.events)) return false;

  const jobIdStr = String(jobId);
  const others = cached.events.filter(
  ...
  writeSchedulerCache(key, {
    ...cached,
    events: [...others, ...incoming],
  });
  return true;
}

/** Ignore our own technician_jobs Realtime echo for this long after a local patch. */
const LOCAL_MUTATION_SKIP_MS = 8000;

/** @type {{ technicianJobId: string, technicianId: string, jobId: string, at: number } | null} */
let lastLocalSchedulerMutation = null;

/** Record a local scheduler write so Realtime can skip our own echo. */
export function markLocalSchedulerMutation(event, { jobId } = {}) {
  if (!event && jobId == null) return;
  const technicianJobId = event?.technicianJobId ?? event?.id ?? event?.event_id;
  const technicianId = event?.technicianId ?? event?.resourceId;
  const resolvedJobId = jobId ?? event?.jobId;
  lastLocalSchedulerMutation = {
    technicianJobId: technicianJobId != null ? String(technicianJobId) : "",
    technicianId: technicianId != null ? String(technicianId) : "",
    jobId: resolvedJobId != null ? String(resolvedJobId) : "",
    at: Date.now(),
  };
}

/**
 * True when a technician_jobs Realtime payload is the echo of our own reassign
 * (or has no row id during the skip window). Other jobs still refresh.
 * Multi-crew reassign skips every row for the same jobId (~8s).
 */
export function shouldSkipOwnTechnicianJobsRealtime(payload) {
  if (!lastLocalSchedulerMutation) return false;
  if (Date.now() - lastLocalSchedulerMutation.at > LOCAL_MUTATION_SKIP_MS) {
    return false;
  }

  const payloadJobId = payload?.new?.job_id ?? payload?.old?.job_id;
  if (
    lastLocalSchedulerMutation.jobId &&
    payloadJobId != null &&
    String(payloadJobId) === lastLocalSchedulerMutation.jobId
  ) {
    return true;
  }

  const rowId = payload?.new?.id ?? payload?.old?.id;
  const newTechId = payload?.new?.technician_id;

  if (rowId == null) {
    return true;
  }
  if (String(rowId) !== lastLocalSchedulerMutation.technicianJobId) {
    return false;
  }
  if (
    newTechId != null &&
    lastLocalSchedulerMutation.technicianId &&
    String(newTechId) !== lastLocalSchedulerMutation.technicianId
  ) {
    return false;
  }
  return true;

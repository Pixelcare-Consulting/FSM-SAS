...
import { emitJobAssignmentEmails } from "../notifications/transactionalJobEmailClient";
import {
...
export const fetchSchedulerTechnicians = async () => {
  const response = await fetch("/api/scheduler/technicians", {
    credentials: "include",
    cache: "no-store",
  });
  if (!response.ok) {
...
export const fetchSchedulerWindowData = async (
  ...
  const response = await fetch(
    `/api/scheduler/technician-data${query ? `?${query}` : ""}`,
    {
      credentials: "include",
      cache: "no-store",
      signal,

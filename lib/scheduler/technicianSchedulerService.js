...
const WINDOW_FETCH_KEY_PREFIX = "/api/scheduler/technician-data";

let windowFetchGeneration = 0;

/**
 * Drop shared in-flight window fetches so a post-mutation request cannot
 * join a pre-mutation technician-data call and apply stale JSON.
 */
export function bumpSchedulerWindowFetchGeneration() {
  windowFetchGeneration += 1;
  for (const key of [...inFlightFetches.keys()]) {
    if (String(key).includes(WINDOW_FETCH_KEY_PREFIX)) {
...
export const fetchSchedulerWindowData = async (
  ...
  const url = `${WINDOW_FETCH_KEY_PREFIX}${query ? `?${query}` : ""}`;
  const requestKey = `${url}#g${windowFetchGeneration}`;
...
const uniqueTechnicianIds = (technicianIds = []) =>
  [...new Set(
...
/**
 * Replace a job’s full technician crew WITHOUT touching appointment times.
 *
 * Selected ids become the crew: overlapping rows are kept, missing ids are
 * soft-deleted, new ids are inserted. Appointment display uses jobs.scheduled_end
 * (canonical); reassign must not write display-derived times back to DB.
 *
 * @returns {Promise<object[]>} scheduler events for the resulting crew
 */
export const reassignTechnicians = async ({ jobId, technicianIds }) => {
  if (!jobId) throw new Error("Job ID is required");

  const selectedIds = uniqueTechnicianIds(technicianIds);
  ...
  const missingIds = selectedIds.filter((id) => !techById.has(id));
  if (missingIds.length) {
    throw new Error("One or more technicians could not be found");
  }
  ...
  const existingTechIds = (existingTechJobs || []).map((row) =>
    String(row.technician_id)
  );
  const selectedIdSet = new Set(selectedIds);
  const techIdsToAdd = selectedIds.filter((id) => !existingTechIds.includes(id));
  const techIdsToRemove = existingTechIds.filter((id) => !selectedIdSet.has(id));
  ...
  if (techIdsToAdd.length > 0) {
    ...
    const newAssignmentStatus = mapJobStatusToAssignmentStatus(jobRow?.status);
    const { error: insertError } = await supabase
      .from("technician_jobs")
      .insert(
        techIdsToAdd.map((technicianId) => ({
          technician_id: technicianId,
          job_id: jobId,
  ...
  const primaryName = techById.get(selectedIds[0])?.full_name || null;
  const primarySchedule = await fetchPrimarySchedule(supabase, jobId);
  let scheduleRecord = primarySchedule;
  if (primarySchedule?.id && primaryName) {
    const { data: updatedSchedule, error: schedError } = await supabase
      .from("job_schedule")
      .update({ job_tech: primaryName })
  ...
  if (techIdsToAdd.length > 0) {
    void emitJobAssignmentEmails({
      jobId,
      technicianIds: techIdsToAdd,
    });
  }

  const { data: resultingAssignments, error: resultingError } = await supabase
    .from("technician_jobs")
  ...
  const assignmentByTechId = new Map(
    (resultingAssignments || []).map((row) => [String(row.technician_id), row])
  );
  const orderedAssignments = selectedIds
    .map((id) => assignmentByTechId.get(id))
    .filter(Boolean);

  return orderedAssignments
    .map((assignment) => buildEventFromAssignment(assignment, scheduleRecord))
    .filter(Boolean);
};

/**
 * Reassign a job to a single technician (full-crew replace with one id).
 * Appointment times are not changed.
 */
export const reassignTechnician = async ({ jobId, technicianId }) => {
  const events = await reassignTechnicians({
    jobId,
    technicianIds: technicianId != null ? [technicianId] : [],
  });
  const match = events.find(
    (evt) => String(evt?.technicianId) === String(technicianId)
  );

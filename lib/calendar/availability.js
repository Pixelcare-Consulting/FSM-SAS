...
export function getTechnicianAvailabilityIssues({
  ...
  if (technicianId) {
    const leaveEvents = getEventsForDate(events, ymd, {
      ...
      (event) =>
        LEAVE_EVENT_TYPES.has(event.eventType) && eventCoversDateTime(event, dateLike)
    );
...
export function technicianHasAllDayLeave(calendarEvents, technicianId, ymd) {
  return getEventsForDate(calendarEvents, ymd, { scope: "technician", technicianId }).some(
    (event) => LEAVE_EVENT_TYPES.has(event.eventType) && isAllDayCalendarEvent(event)
  );
}

export function technicianLeaveCoversSlot(
  calendarEvents,
  technicianId,
  slotStart,
  slotMinutes = 30
) {
  if (!technicianId || !slotStart) return false;
  return normalizeEvents(calendarEvents).some(
    (event) =>
      event.scope === "technician" &&
      event.technicianId === technicianId &&
      LEAVE_EVENT_TYPES.has(event.eventType) &&
      eventCoversTimeSlot(event, slotStart, slotMinutes)
  );
}

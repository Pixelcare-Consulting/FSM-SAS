...
import { buildExecJobIncentiveData } from "../../../lib/sap/jobIncentiveSpSql";
...
export default async function handler(req, res) {
  ...
  if (!Number.isFinite(year) || !Number.isFinite(quarter) || !Number.isFinite(month)) {
    return res.status(400).json({ success: false, error: "Invalid year, quarter, or month", rows: [] });
  }

  const sql = buildExecJobIncentiveData(filterType, year, quarter, month);
  ...
  try {
    ...
    return res.status(200).json({
      success: true,
      rows,
      meta: { filterType, year, quarter, month },
    });
  } catch (error) {
    console.error("SAP _SPPXC_Job_Incentive_Data error:", error);

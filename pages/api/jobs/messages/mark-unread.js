...
export default withSession(async function handler(req, res, session) {
  ...
  try {
    ...
    let result;
    if (jobIdStr) {
      result = await markJobThreadUnread(supabase, userId, jobIdStr);
    } else {
      result = await markJobMessagesUnread(supabase, userId, idList);
    }
    ...
    console.error('jobs/messages/mark-unread API error:', error);
    return res.status(500).json({
      error: error.message || 'Unable to mark messages as unread.',
    });
  }
});

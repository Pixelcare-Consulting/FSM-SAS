...
export default function CalendarEventForm({
  ...
  useEffect(() => {
    ...
    setError(null);
    if (initialEvent) {
      const allDay = initialEvent.allDay !== false;
      setForm({
        ...
        allDay,
        startTime: allDay ? "" : normalizeCalendarTimeHm(initialEvent.startTime) || "",
        endTime: allDay ? "" : normalizeCalendarTimeHm(initialEvent.endTime) || "",
  ...
  const handleAllDayChange = (event) => {
    ...
    setForm((prev) => ({
      ...prev,
      allDay,
      startTime: allDay ? "" : prev.startTime || "08:00",
      endTime: allDay ? "" : prev.endTime || "13:00",
    }));
  };
  ...
  const handleSubmit = async (event) => {
    ...
    const isTechnicianLeave = form.scope === CALENDAR_SCOPES.TECHNICIAN;
    const allDay = !isTechnicianLeave || form.allDay;
    if (isTechnicianLeave && !allDay) {
      const startTime = normalizeCalendarTimeHm(form.startTime);
      const endTime = normalizeCalendarTimeHm(form.endTime);
      if (!startTime || !endTime) {
        setError("Please select start and end times");
        return;
      }
      if (form.startDate === (form.endDate || form.startDate) && endTime <= startTime) {
    ...
    try {
      const payload = {
        ...
        allDay,
        startTime: allDay ? null : normalizeCalendarTimeHm(form.startTime),
        endTime: allDay ? null : normalizeCalendarTimeHm(form.endTime),
      };
      ...
      <Row className="g-3">
        ...
        {isTechnicianLeave && (
          <Col md={12}>
            ...
            <Form.Text>
              {form.allDay
                ? "Uncheck All day to set morning or afternoon hours (for example 08:00–13:00)."
                : "Leave covers only the hours you set. Other hours stay available on the scheduler."}
            </Form.Text>
          </Col>
        )}
        ...
        {isTechnicianLeave ? (
          <Col md={6}>
            <Form.Label htmlFor="calendar-event-start-time">Start time</Form.Label>
            <Form.Control
        ...
        {isTechnicianLeave ? (
          <Col md={6}>
            <Form.Label htmlFor="calendar-event-end-time">End time</Form.Label>
            <Form.Control
        ...
        {isSingleDay && !isTechnicianLeave && (
          <Col md={12}>
            <Form.Text>Single day event</Form.Text>
          </Col>
        )}
        {isSingleDay && isTechnicianLeave && form.allDay && (
          <Col md={12}>
            <Form.Text>Single day leave</Form.Text>
          </Col>
        )}

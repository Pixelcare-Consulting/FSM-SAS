...
const ViewLeads = () => {
  ...
  const loadData = useCallback(async (page = 1) => {
    ...
    try {
      ...
      const response = await fetch(`/api/getLeadsList?${params.toString()}`, {
        credentials: 'include',
        headers: {
      ...
      if (isMountedRef.current) {
        setRawData(leads);
        setTotalRows(result.totalCount || leads.length);
      }
    } catch (err) {
  ...
  return (
    <Fragment>
      <GeeksSEO title="SAP Leads (live SAP API) | SAS&ME Portal" />
      <Row>
            ...
            <div className="d-flex justify-content-between align-items-start">
              <div className="d-flex flex-column">
                <div className="mb-3">
                  ...
                  >
                    Legacy list from SAP Business One Service Layer (requires SAP session). Prefer the{' '}
                    <Link href="/leads" className="text-white text-decoration-underline fw-semibold">
                      Supabase masterlist
                    </Link>
                    .
                  </p>
                  ...
                  >
                    ...
                    <span>
                      View-only. Lead data is maintained in SAP Business One (this page pulls live API).
                    </span>
                  </div>
                </div>

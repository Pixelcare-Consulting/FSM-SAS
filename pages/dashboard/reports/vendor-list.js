...
const VendorListPage = () => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/reports/vendor-list?${params.toString()}`);
  ...
  return (
    <ReportPageShell
      title="Vendor List"
      subtitle="SAP vendors are not stored in this database — showing customers (business partners) from Supabase instead"
      headerRight={
    ...
    >
      <Alert variant="info" className="mb-3 py-2" style={{ fontSize: 13 }}>
        There is no <code>vendors</code> table in the FSM schema. This report lists <strong>customers</strong> so you still have a
        searchable directory in the portal.
      </Alert>
      ...
      <Card style={{ borderRadius: 12, border: "1px solid #e2e8f0" }}>
        <Card.Header className="bg-white py-3 px-4" style={{ borderBottom: "1px solid #e2e8f0", borderRadius: "12px 12px 0 0" }}>
          <h6 style={{ fontWeight: 700, margin: 0, color: "#1e293b" }}>Customers ({filtered.length})</h6>
        </Card.Header>

...
const ViewCustomers = () => {
  ...
  } = useCustomersListQuery(customersQueryParams);

  const rawData = customersData?.customers || [];
  const data = rawData;
  const totalRows = customersData?.totalCount ?? rawData.length;
  const customerCount = customersData?.totalCount ?? rawData.length;
  const estimatedTotal = customersData?.totalCount ?? 0;
  const addressCount = customersData?.addressCount ?? 0;
  const topCountry = customersData?.topCountry || '';
  const topCountryCount = customersData?.topCountryCount || 0;

  useEffect(() => {
    setLoading(customersLoading || customersFetching);
    if (customersQueryError) {
      setError(customersQueryError.message || 'Failed to load customers. Please try again.');
    } else if (customersData) {
      setError(null);
    }
  }, [customersLoading, customersFetching, customersQueryError, customersData]);

  useEffect(() => {
    if (customersData && !customersFetching) {
      setInitialLoad(false);
    }
  }, [customersData, customersFetching]);
          ...
          <div style={{ cursor: 'default' }}>
            ...
            {primaryAddress ? (
              ...
              (() => {
                ...
                const addressCount = row.locationCount ?? allAddresses.length;
  ...
  const table = useReactTable({
    ...
    // Server-side pagination via masterlist-summary API
  ...
  const loadData = useCallback(async (page = 1) => {
    ...
    setLoadingStep(1);
    setLoadingMessages(['Loading customers…', 'Server-paginated summary']);
    ...
    try {
      ...
      setLoadingMessages([
        `Loaded page ${page} (${customers.length.toLocaleString()} rows)`,
        `${(payload?.totalCount ?? 0).toLocaleString()} customers total`,
      ]);
      appendLoadingLog(`API: ${customers.length} customers (page ${page})`);
    } catch (err) {
  ...
  }, [appendLoadingLog, currentPage, refetchCustomers]);
  ...
  const {
    ...
    onSyncSuccess: async ({ summary, normalizedCode, loadingToastId }) => {
      ...
      const customersWritten =
        (summary.counts?.masterlistCustomersInserted || 0) +
        (summary.counts?.masterlistCustomersUpdated || 0);
      const wasPromotion = summary.mode === 'promotion';

      if (normalizedCode && !wasPromotion && customersWritten === 0) {
        const detail =
          summaryErrors[0] ||
          `No customer row was written for ${normalizedCode}. Verify the CardCode (e.g. C006104, not C008104).`;
  ...
  return (
    <Fragment>
      ...
      />
      <GeeksSEO title="Customers | SAS&ME Masterlist | Portal" />
      <Row>
      ...
      {(loading || initialLoad) && (
        <CustomerListLoadingIndicator
          ...
          onCancel={null}
          title="Loading customer masterlist"
          showStepsSection={false}
          showTipsSection={false}
          footerPrimary="Reading customers and service locations from Supabase…"
          footerSecondary="Large lists are streamed in batches; this is usually much faster than the SAP service layer."
        />
      )}
